...
class CampDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.target_transform: lbl = self.target_transform(lbl)
        return img, lbl

class CampDataModule(pl.LightningDataModule):
    def __init__(self, image_dir, label_dir, batch_size=32, val_ratio=0.15, test_ratio=0.15, seed=62):
        super().__init__()
        self.image_dir, self.label_dir = image_dir, label_dir
        self.batch_size, self.val_ratio, self.test_ratio, self.seed = batch_size, val_ratio, test_ratio, seed
    ...
    def setup(self, stage=None):
        ...
        n = len(full)
        t = int(n*self.test_ratio); v = int(n*self.val_ratio)
        train, val, test = random_split(full, [n-v-t, v, t], generator=torch.Generator().manual_seed(self.seed))
...
class RefugeeCampDetector(nn.Module):
    def __init__(self):
        super().__init__()
        self.conv1 = nn.Conv2d(3,64,3, padding=1); self.pool1 = nn.MaxPool2d(2)
        self.conv2 = nn.Conv2d(64,128,3, padding=1); self.pool2 = nn.MaxPool2d(2)
        self.conv3 = nn.Conv2d(128,256,3, padding=1); self.pool3 = nn.MaxPool2d(2)
    ...
    def forward(self, x):
        c1, p1 = F.relu(self.conv1(x)), self.pool1(F.relu(self.conv1(x)))
        c2, p2 = F.relu(self.conv2(p1)), self.pool2(F.relu(self.conv2(p1)))
        c3, p3 = F.relu(self.conv3(p2)), self.pool3(F.relu(self.conv3(p2)))
