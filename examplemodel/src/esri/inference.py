...
class RefugeeCampDetector:
    ...
    def getParameterInfo(self):
        required_parameters = [
            {'name': 'raster', 'dataType': 'raster', 'required': True, 'displayName': 'Raster', 'description': 'Input Raster'},
            {'name': 'model', 'dataType': 'string', 'required': True, 'displayName': 'Input Model Definition (EMD) File', 'description': 'Input model definition (EMD) JSON file'},
            {'name': 'device', 'dataType': 'numeric', 'required': False, 'displayName': 'Device ID', 'description': 'Device ID'},
            {'name': 'batch_size', 'dataType': 'numeric', 'required': False, 'value': 1 if "BatchSize" not in self.json_info else int(self.json_info["BatchSize"]), 'displayName': 'Batch Size', 'description': 'Batch Size'},
            {'name': 'threshold', 'dataType': 'numeric', 'required': False, 'value': 0.5 if "Threshold" not in self.json_info else float(self.json_info["Threshold"]), 'displayName': 'Confidence Score Threshold [0.0, 1.0]', 'description': 'Confidence score threshold value [0.0, 1.0]'}
        ]
        return required_parameters

    def getConfiguration(self, **scalars):
        self.batch_size = int(scalars.get("batch_size", self.json_info.get("BatchSize", 1)))
        self.thres = float(scalars.get("threshold", self.json_info.get("Threshold", 0.5)))
    ...
    def getFields(self):
        fields = [
            ...
            {'name': 'Classvalue', 'type': 'esriFieldTypeInteger', 'alias': 'Classvalue'},
